...
- v2 的统一媒体数据库升级到 v3 时只增加 `anime.merged_into`、
  `episode.merged_into` 和对应索引，不删除或重建已有表。
  旧 `watch_history.db` 不属于这个升级路径，传错数据库时会拒绝升级。
- 合并通用 Anime/Episode 会保留旧记录，并用 `merged_into` 指向目标。
  旧 ID 不会被自动分配给其他实体。通过共通 ID 查询来源时会解析重定向，
  枚举共通实体时只返回合并后的有效记录。
- 合并在一个数据库事务中迁移整个通用实体的来源引用；Anime 合并迁移其
  Episode，Episode 合并迁移其视频资产。目标中已有不同的同类来源记录，
  或 Episode 属于不同 Anime 时，操作失败并回滚，不覆盖或拆散原有关系。
- 单独重新匹配视频只移动该视频；旧 Episode 还有其他视频或来源引用时保留
  原身份。最后一个引用离开无来源的临时 Episode 时，为它保留重定向。
- `anime/{id}.json` 和 `episode/{id}.json` 必须通过
  `readJsonFromFile()` / `saveJsonToFile()` 读写。读取合并后的 ID 也会读取
  旧 ID 对应的 JSON，原文件保留。字段冲突时较接近最终目标的文件优先，
  同一深度按 ID 升序合并；嵌套对象递归合并，其他值采用目标值。
  ID 必须是数据库中已经存在的通用记录，避免提前写出的文件与未来 ID 冲突。
- 用户 JSON 的保存按字段更新，在解析、编码成功后写入同目录临时文件，
  再用 rename 替换目标。读取、保存与 ID 合并由数据库事务串行化，避免
  保存过程中 ID 改变。损坏的源 JSON 会使操作报错，不静默覆盖。
- 资产 upsert 中省略的大小、编码和 SHA-256 表示“本次未提供”，保留原值。
  该接口不把省略字段解释为清除操作。

这些规则只涉及新统一媒体数据库和它的元数据，不启用新数据库的业务接入，
也不迁移旧观看历史、收藏、同步文件或用户配置。
...
这些 JSON 文件的内容不属于 SQLite schema，数据库回滚不会回滚文件内容。
v3 的通用 JSON 读写会与 ID 合并串行化；合并本身只更新数据库中的重定向，
不搬动或删除 JSON 文件。
