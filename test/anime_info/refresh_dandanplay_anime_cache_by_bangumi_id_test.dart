...
// test/anime_info/refresh_dandanplay_anime_cache_by_bangumi_id_test.dart
...
void main() {
  ...
  test('刷新 Dandanplay Anime Info 到 JSON 缓存', () async {
    ...
    final bangumiTvAnimeId = getIntFromEnv(TestEnvironmentVariables.bangumiTvAnimeId);
    final databasePath = getStringFromEnv(TestEnvironmentVariables.databasePath);
    if (bangumiTvAnimeId == null || databasePath == null) {
      printMsg(color('测试未运行', ColorCode.red));
      return;
    }

    await DatabaseService.initialize(databasePath);
    await AnimeInfoService.refreshDandanplayAnimeCacheByBangumiId(bangumiTvAnimeId);
  });
}
