...
class PagePrewarmer {
  ...
  Future<void> _checkAndFixAnimeCache() async {
    try {
        ...
        try {
          ...
          if (cachedString != null) {
            ...
            if (data.containsKey('animeDetail')) {
              ...
              // 检查剧集列表格式
              if (animeData.containsKey('episodeList') &&
                  animeData['episodeList'] is List) {
                final episodeList = animeData['episodeList'] as List;
                if (episodeList.isNotEmpty) {
                  // 检查第一个剧集的格式
                  final firstEpisode = episodeList.first;
                  if (firstEpisode is Map &&
                      (firstEpisode.containsKey('id') &&
                          !firstEpisode.containsKey('episodeId'))) {
                    needsFix = true;
                  }
                }
              }
      ...
      if (needRefreshAnimeIds.isNotEmpty) {
        ...
        final animeIdsToRefresh =
            needRefreshAnimeIds.take(refreshLimit).toList();
  ...
  /// 检查并修复单个历史项
  Future<void> _checkAndFixHistoryItem(WatchHistoryItem historyItem,
      WatchHistoryProvider historyProvider) async {
    // 检查是否缺少关键信息
    ...
    if (needsFixing) {
      ...
      debugPrint(
          '[页面预热] 当前信息: animeId=${historyItem.animeId}, episodeId=${historyItem.episodeId}');
      ...
      try {
        ...
        if (filePath.isNotEmpty && File(filePath).existsSync()) {
          // 重新匹配视频信息
          final videoMatchInfo =
              await DanmakuMatchingService.instance.getVideoInfo(filePath);

          if (videoMatchInfo['success'] == true &&
              videoMatchInfo['matches'] != null) {
            ...
            if (matches.isNotEmpty) {
              ...
              if (animeId != null &&
                ...
                debugPrint(
                    '[页面预热] 成功修复历史项: animeId=$animeId, episodeId=$episodeId');
            ...
            debugPrint(
                '[页面预热] 匹配视频信息失败: ${videoMatchInfo['errorMessage'] ?? '未知错误'}');
          }
        } else {
      ...
      // 检查是否需要刷新番剧详情缓存
      final hasCache = await BangumiService.instance
          .hasCachedAnimeDetails(historyItem.animeId!);
